...
const Home = () => {
  ...
  return (
    <div className={`min-h-[80vh] ${isDarkMode ? 'bg-slate-900 text-white' : 'bg-gradient-to-br from-slate-50 to-emerald-100'}`}>
      <div className="h-10" />
      {/* Slider */}
      ...
      <div className="max-w-6xl mx-auto mt-16 mb-12 px-4">
        <h2 className={`text-2xl font-bold mb-6 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-700'}`}>Featured Tasks</h2>
        {loading ? (
          <div className={`text-center font-semibold ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>Loading...</div>
        ) : featured.length === 0 ? (
          <div className={`text-center ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>No featured tasks found.</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {featured.map((task) => (
              <div key={task._id} className={`rounded-xl shadow-lg p-6 flex flex-col justify-between hover:shadow-xl transition-all duration-300 ${
      ...
      <div className="max-w-6xl mx-auto mb-16 px-4">
        <h2 className={`text-2xl font-bold mb-6 ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>Browse by Category</h2>
      ...
      <div className={`py-12 ${isDarkMode ? 'bg-slate-800' : 'bg-white'}`}>
        <div className="max-w-6xl mx-auto px-4">
          <h2 className={`text-2xl font-bold mb-8 text-center ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>How It Works</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="flex flex-col items-center text-center">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${
                isDarkMode ? 'bg-emerald-900' : 'bg-emerald-100'
              }`}>
                <span className={`text-3xl font-bold ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>1</span>
              </div>
              <h3 className={`font-semibold text-lg mb-2 ${isDarkMode ? 'text-slate-200' : ''}`}>Post a Task</h3>
              <p className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>Describe your project and requirements. It's free and easy to post a task.</p>
            </div>
            <div className="flex flex-col items-center text-center">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${
                isDarkMode ? 'bg-emerald-900' : 'bg-emerald-100'
              }`}>
                <span className={`text-3xl font-bold ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>2</span>
              </div>
              <h3 className={`font-semibold text-lg mb-2 ${isDarkMode ? 'text-slate-200' : ''}`}>Receive Bids</h3>
              <p className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>Freelancers will bid on your task. Review profiles, chat, and choose the best fit.</p>
            </div>
            <div className="flex flex-col items-center text-center">
              <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${
                isDarkMode ? 'bg-emerald-900' : 'bg-emerald-100'
              }`}>
                <span className={`text-3xl font-bold ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>3</span>
              </div>
              <h3 className={`font-semibold text-lg mb-2 ${isDarkMode ? 'text-slate-200' : ''}`}>Get It Done</h3>
              <p className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>Collaborate, track progress, and pay securely when the work is complete.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`py-12 ${isDarkMode ? 'bg-gradient-to-r from-emerald-900 to-teal-900' : 'bg-gradient-to-r from-emerald-50 to-teal-50'}`}>
        <div className="max-w-6xl mx-auto px-4">
          <h2 className={`text-2xl font-bold mb-8 text-center ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>Why Choose TalentForge?</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className={`rounded-xl shadow p-6 flex flex-col items-center text-center ${
              isDarkMode ? 'bg-slate-800' : 'bg-white'
            }`}>
              <span className={`text-3xl mb-2 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>🔒</span>
              <h3 className={`font-semibold text-lg mb-2 ${isDarkMode ? 'text-slate-200' : ''}`}>Secure Payments</h3>
              <p className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>Your money is safe until you approve the work. We use trusted payment gateways.</p>
            </div>
            <div className={`rounded-xl shadow p-6 flex flex-col items-center text-center ${
              isDarkMode ? 'bg-slate-800' : 'bg-white'
            }`}>
              <span className={`text-3xl mb-2 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>🌟</span>
              <h3 className={`font-semibold text-lg mb-2 ${isDarkMode ? 'text-slate-200' : ''}`}>Quality Freelancers</h3>
              <p className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>All freelancers are vetted and reviewed. Find the right talent for any job.</p>
            </div>
            <div className={`rounded-xl shadow p-6 flex flex-col items-center text-center ${
              isDarkMode ? 'bg-slate-800' : 'bg-white'
            }`}>
              <span className={`text-3xl mb-2 ${isDarkMode ? 'text-emerald-400' : 'text-emerald-600'}`}>💬</span>
              <h3 className={`font-semibold text-lg mb-2 ${isDarkMode ? 'text-slate-200' : ''}`}>24/7 Support</h3>
              <p className={isDarkMode ? 'text-slate-400' : 'text-slate-600'}>Our support team is always here to help you, day or night.</p>
            </div>
          </div>
        </div>
      </div>
