...
const Profile = () => {
  ...
  return (
    <div className={`min-h-[80vh] flex justify-center items-center p-4 ${isDarkMode ? 'bg-slate-900 text-white' : 'bg-gradient-to-br from-slate-50 to-emerald-100 text-slate-900'}`}>
      <div className={`rounded-2xl shadow-xl border p-8 max-w-md w-full flex flex-col items-center relative ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
        {!editMode && (
          <button
          ...
          >
            ...
            <div className="flex gap-2 mt-2">
              ...
              <button
                ...
                className={`flex items-center gap-1 px-4 py-2 rounded-lg font-semibold transition-colors ${isDarkMode ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}
              >
                <FaTimes /> Cancel
              </button>
            </div>
            {error && <div className={`text-red-500 text-sm mt-2 ${isDarkMode ? 'text-red-300' : ''}`}>{error}</div>}
            {success && (
              <div className={`text-emerald-600 text-sm mt-2 ${isDarkMode ? 'text-emerald-400' : ''}`}>{success}</div>
            )}
          </form>
        ) : (
          <>
            <h2 className={`text-2xl font-bold text-emerald-700 mb-2 ${isDarkMode ? 'text-white' : ''}`}>
              {user.displayName || "No Name"}
            </h2>
            <div className={`text-slate-600 text-sm break-all text-center mb-1 ${isDarkMode ? 'text-white' : ''}`}>
